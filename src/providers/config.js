...
export function resolveModelName(envName, fallback, env = process.env) {
...
export function resolveChairProvider(providers, chairName, env = process.env) {
  const name = String(chairName || env.CHAIR_PROVIDER || 'openai').toLowerCase();
  ...
  const chairModel = String(env.CHAIR_MODEL || '').trim();
  if (chairModel === 'undefined' || chairModel === 'null' || /\s/.test(env.CHAIR_MODEL || '')) {
